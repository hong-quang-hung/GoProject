...
func Leetcode_Pair_Sum() {
	...
	fmt.Println("Output:", pairSum(utils.S2ListNode("[5,4,2,1]")))
	fmt.Println("Input: head = [4,2,2,3]")
	fmt.Println("Output:", pairSum(utils.S2ListNode("[4,2,2,3]")))
	fmt.Println("Input: head = [1,100000]")
	fmt.Println("Output:", pairSum(utils.S2ListNode("[1,100000]")))
}

func pairSum(head *types.ListNode) int {
	fast, slow := head, head
